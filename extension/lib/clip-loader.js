...
export async function loadAllClips(channelSlug, fetchPage, onProgress, signal, options = {}) {
  const clipMap = new Map();

  if (options.initialClips?.length) {
  ...
  let shownPartial = clipMap.size > 0;
  let knownTotalCount = options.knownTotalCount ?? null;

  if (knownTotalCount != null && clipMap.size >= knownTotalCount) {
    knownTotalCount = null;
  }

  let stagnantPages = 0;
  let prefetchPromise = null;
  let pendingNextCursor = null;
  ...
  try {
    while (true) {
      ...
      const belowKnownTotal = knownTotalCount == null || loaded < knownTotalCount;
    ...
    if (error instanceof LoadCancelledError) {
      ...
      const canResume = pendingNextCursor != null && pendingNextCursor !== '';
      onProgress({
        phase: 'cancelled',
        loaded: clips.length,
        clips,
        hasMore: canResume,
        nextCursor: canResume ? pendingNextCursor : null,
        totalCount: knownTotalCount ?? clips.length,
        message: ''
      });
      return clips;
    }
