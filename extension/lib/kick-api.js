...
function clipSortValue(clip, sortMode) {
  switch (sortMode) {
    case 'popular':
    case 'unpopular':
      return Number(clip.view_count) || 0;
    case 'newest':
    case 'oldest':
    default:
      return Date.parse(clip.created_at) || 0;
  }
}
...
export function sortClips(clips, sortMode) {
  ...
  const mode = sortMode === 'newest' || sortMode === 'popular' ? sortMode : (
    sortMode === 'unpopular' ? 'unpopular' : 'oldest'
  );
  const entries = new Array(clips.length);

  for (let index = 0; index < clips.length; index += 1) {
    entries[index] = {
      clip: clips[index],
      value: clipSortValue(clips[index], mode)
    };
  }

  const descending = mode === 'newest' || mode === 'popular';
  entries.sort((a, b) => (descending ? b.value - a.value : a.value - b.value));

  const sorted = new Array(entries.length);
  for (let index = 0; index < entries.length; index += 1) {
    sorted[index] = entries[index].clip;
  }
